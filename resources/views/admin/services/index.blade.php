        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="flex flex-wrap items-center justify-end gap-4 p-5 border-b border-gray-100 bg-gray-50/50">
                <a href="{{ route('admin.services.create') }}"
                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-medium rounded-xl shadow-lg shadow-blue-500/25 transition-all duration-200 transform hover:scale-[1.02]">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($services as $service)
                            <tr class="hover:bg-gray-50/50 transition-colors group">
                                        ...
                                        @if ($service->icon)
                                            <div class="flex items-center gap-1 mt-1">
                                                ...
                                                <span class="text-sm"><i
                                                        class="{{ $service->icon ?? 'fas fa-cogs' }} text-sm"></i></span>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $service->is_active ? 'bg-green-50 text-green-700' : 'bg-yellow-50 text-yellow-700' }}">
                                        <span
                                            class="w-1.5 h-1.5 rounded-full {{ $service->is_active ? 'bg-green-500' : 'bg-yellow-500' }}"></span>
                                        {{ $service->is_active ? 'Aktif' : 'Nonaktif' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $service->is_featured ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-500' }}">
                                        <svg class="w-3 h-3 {{ $service->is_featured ? 'text-amber-500' : 'text-gray-400' }}"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-1">
                                        ...
                                        @endif
                                        <span
                                            class="text-sm font-mono text-gray-500 min-w-[2rem] text-center">{{ $service->sort_order }}</span>
                                        @if (!$loop->last)
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.services.edit', $service) }}"
                                            class="p-2 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-all duration-200"
                                            title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex-1">
                                ...
                                <div class="mt-3">
                                    ...
                                    <div class="mt-4 p-3 bg-red-50 rounded-xl border border-red-100">
                                        <div class="flex items-start gap-2">
                                            ...
                                            <p class="text-xs text-red-700">
                                                Tindakan ini akan menghapus layanan secara permanen dan tidak dapat
                                                dibatalkan.
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-4 space-y-2">
                                        <p class="text-xs font-semibold text-gray-700 uppercase tracking-wider">Data yang
                                            akan dihapus:</p>
