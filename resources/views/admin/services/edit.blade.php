...
<div class="max-w-7xl mx-auto px-4 sm:px-0">
    <form method="POST" action="{{ route('admin.services.update', $service) }}" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">Edit Layanan</h1>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5 hover:shadow-md transition-shadow">
                    ...
                    <div>
                        ...
                        <div class="flex justify-between items-center mt-1">
                            ...
                            <p class="text-xs text-gray-400" id="charCount">{{ strlen($service->description) }} karakter</p>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl" id="previewCard">
                        <div class="text-center space-y-3">
                            <div class="text-5xl" id="previewIcon">{{ $service->icon ?: '📦' }}</div>
                            <h4 class="font-bold text-gray-800" id="previewTitle">{{ $service->title }}</h4>
                            <p class="text-sm text-gray-600 line-clamp-3" id="previewDescription">{{ Str::limit($service->description, 100) }}</p>
                        </div>
                    </div>
...
function updatePreview() {
    ...
    else previewTitle.textContent = '{{ $service->title }}';

    if (descriptionInput.value) previewDescription.textContent = descriptionInput.value;
    else previewDescription.textContent = '{{ Str::limit($service->description, 100) }}';

    if (iconInput.value) previewIcon.textContent = iconInput.value;
    else previewIcon.textContent = '{{ $service->icon ?: '📦' }}';
}
